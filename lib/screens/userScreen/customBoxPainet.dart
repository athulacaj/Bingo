...
class MyCustomBoxPaint extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (data.containsKey("labelV")) {
      setPoint = Offset(width / 2 - 3, height + 3);
      paintLabelPoint(canvas, setPoint, data['labelV']);
    }
    if (data.containsKey("labelH")) {
      paintLabelPoint(
          canvas, Offset(width + 3, height / 2 - 13), data['labelH']);
    }
    if (data.containsKey("labelD")) {
      paintLabelPoint(canvas, Offset(width + 3, height + 3), data['labelD']);
    }
    if (data.containsKey("labelO")) {
      paintLabelPoint(canvas, Offset(-10, height + 3), data['labelO']);
    }
  ...
  void drawCross(
    ...
    var paint = Paint()
      ..color = isRecentlySelected ? topColor : Colors.black45
      ..strokeWidth = 6
      ..style = PaintingStyle.stroke
